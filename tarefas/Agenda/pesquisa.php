...
require_once('data_base_conection.php');
  ...
  $sql="SELECT * FROM peoples where nome = '$pesquisa' AND user_id = '$id_user';";



     $dados_usuario=mysqli_query($link, $sql);

     $dados_usuario= mysqli_fetch_array($dados_usuario);


      if(!isset($dados_usuario['nome'])){ //nao existe dados

        header('Location:home.php?erro=4&pagina=1'); //retorno um erro;
      }
            ...
            <h3>Editar Contato</h3>
                <form method="post" action="editar.php" >
                  ...
                  <div class="form-group" id="deixarculto">
                    <input type="text" class="form-control"  name="id" id="nome" value="<?php echo $dados_usuario['id'] ?>">
                  </div>
                  <div class="form-group">
                    <label for="nome">Nome:</label>
                    <input type="text" class="form-control"  name="nome" id="nome" value="<?php echo $dados_usuario['nome'] ?>">
                  </div>
                   <div class="form-group">
                    <label for="email">E-mail:</label>
                    <input type="email" class="form-control"   name="email" id="email" value="<?php echo $dados_usuario['email'] ?>">
                  </div>  
                   <div class="form-group">
                    <label for="telefone">Telefone:</label>
                    <input type="text" class="form-control"   name="telefone" id="telefone" value="<?php echo $dados_usuario['phone'] ?>">
                   </div>
                  <div class="form-group">
                     <label for="obsercacao">Observação:</label> 
                     <textarea class="form-control" rows="4"  id="obsercacao" name="obsercacao" value="<?php echo $dados_usuario['obsercacao'] ?>"></textarea>
                  </div>
